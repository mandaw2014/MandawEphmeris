...
if ( $wp_version <= 4.9 ) {
	...
	function ams_register_rest_field_for_custom_post_type(){
			$ams_cutom_post_types = array_values(get_post_types(array('public' => true,'exclude_from_search' => false,'_builtin' => false),'names','and'));
			if (($key = array_search('product', $ams_cutom_post_types)) !== false) { // We don't need this field for products
				unset($array[$key]);
			}
			register_rest_field(
			$ams_cutom_post_types, //['product','course','projects']
			'featured_image_src',
			array(
				'get_callback'    => 'ams_get_image_src',
				'update_callback' => null,
